...
TEST_CASE("UnpicklingFail", "[GroupSessionTest]") {
  REQUIRE_THROWS(megolm::group_session_from_pickle("", PICKLE_KEY));
  REQUIRE_THROWS(megolm::inbound_group_session_from_pickle("", PICKLE_KEY));
}
...
TEST_CASE("DecryptionFail", "[GroupSessionTest]") {
  ...
  REQUIRE_THROWS(inbound->decrypt(*message));
}
