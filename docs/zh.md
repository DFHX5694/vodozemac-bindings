...
| C++ 工具链 | Rust 目标 | 静态库 |
|------------|-----------|--------|
| MSVC / clang-cl（`get_argument_syntax() == 'msvc'`） | `x86_64-pc-windows-msvc`（默认） | `<build>/cargo/release/vodozemac.lib` |
| MinGW GCC（如 MSYS2 UCRT64） | `x86_64-pc-windows-gnu` | `<build>/cargo/x86_64-pc-windows-gnu/release/libvodozemac.a` |
...
通过 Meson 构建时（独立项目或作为子项目），Cargo 产物位于**本项目的 Meson 构建目录**（`<build>/cargo/`），不会写入源码树。Cargo 通过 `--manifest-path` 调用，因此在主项目的构建目录中执行也不会找错 `Cargo.toml`。
...
Meson 将 `<build>/cargo/cxxbridge` 加入 include 路径，并链接 Cargo 构建的静态库。MinGW 场景下所需的 Windows 系统库（`ws2_32`、`bcrypt` 等）会自动附加到链接参数。
...
Meson 构建后，类型定义于 `<build>/cargo/cxxbridge/vodozemac/src/lib.rs.h`（手动 `cargo build` 则在 `target/cxxbridge/...`）。
